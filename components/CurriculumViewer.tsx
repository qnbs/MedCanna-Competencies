...
import { ChevronDown, ChevronRight, Book, Search, X, ArrowLeft, Menu, Target, Lightbulb, FlaskConical, BookOpen, BadgeCheck } from 'lucide-react';
        ...
        {activeDomain ? (
            <>
                ...
                <div className="space-y-4 pb-10">
                    ...
                    {/* Learning Objectives */}
                    {activeDomain.learningObjectives && activeDomain.learningObjectives.length > 0 && (
                      <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4 mb-2">
                        <h4 className="font-semibold text-indigo-800 flex items-center gap-2 mb-2 text-sm">
                          <Target className="w-4 h-4" />
                          {language === 'de' ? 'Lernziele' : 'Learning Objectives'}
                        </h4>
                        <ul className="space-y-1.5">
                          {activeDomain.learningObjectives.map((obj, idx) => (
                    ...
                    {/* Key Takeaways */}
                    {activeDomain.keyTakeaways && activeDomain.keyTakeaways.length > 0 && (
                      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 mb-2">
                        <h4 className="font-semibold text-emerald-800 flex items-center gap-2 mb-2 text-sm">
                          <Lightbulb className="w-4 h-4" />
                          {language === 'de' ? 'Kernaussagen' : 'Key Takeaways'}
                        </h4>
                        <ul className="space-y-1.5">
                          {activeDomain.keyTakeaways.map((ta, idx) => (
                    ...
                    return (
                        ...
                        >
                            ...
                            {point.evidenceLevel && (
                              <span className="inline-flex items-center gap-1 mt-2 px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-medium rounded-full">
                                <BadgeCheck className="w-3 h-3" /> {point.evidenceLevel}
                              </span>
                            )}
                            </div>
                            {hasSubPoints && (
                        ...
                        {hasSubPoints && isExpanded && (
                            ...
                            <div className="space-y-3 pt-3 border-t border-slate-100">
                                ...
                                <div key={sIdx} className="flex flex-col gap-1">
                                    ...
                                    {(sub.evidenceLevel || sub.citation) && (
                                      <div className="ml-9 flex flex-wrap items-center gap-2 mt-0.5">
                                        {sub.evidenceLevel && (
                                          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-blue-50 text-blue-600 text-[10px] font-medium rounded">
                                            <BadgeCheck className="w-2.5 h-2.5" /> {sub.evidenceLevel}
                                          </span>
                                        )}
                                        {sub.citation && (
                                          <span className="text-[10px] text-slate-400 italic flex items-center gap-1">
                                            <BookOpen className="w-2.5 h-2.5" /> {sub.citation}
                                          </span>
                                        )}
                                      </div>
                                    )}
                            ...
                            {point.clinicalPearl && (
                              <div className="mt-4 bg-amber-50 border border-amber-200 rounded-lg p-3">
                                <h5 className="font-semibold text-amber-800 flex items-center gap-1.5 text-xs mb-1">
                                  <FlaskConical className="w-3.5 h-3.5" />
                                  {language === 'de' ? 'Klinischer Hinweis' : 'Clinical Pearl'}
                                </h5>
                                <p className="text-amber-700 text-xs leading-relaxed">{point.clinicalPearl}</p>
                              </div>
                            )}

                            {/* Citation */}
                            {point.citation && (
                              <div className="mt-2 flex items-start gap-1.5 text-[11px] text-slate-400 italic">
                        ...
                        {/* Clinical Pearl & Citation for points WITHOUT subpoints */}
                        {!hasSubPoints && (point.clinicalPearl || point.citation) && (
                          <div className="px-4 pb-4 pl-14">
                            {point.clinicalPearl && (
